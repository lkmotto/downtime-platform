...
export function EventCard({ event, onOpen, variant = "default" }: EventCardProps) {
  ...
  return (
    <div
      ...
      onClick={() => onOpen(event)}
      className="rounded-xl overflow-hidden bg-card border border-card-border cursor-pointer group hover:border-muted-foreground/20 transition-all duration-200"
    >
      {/* Image/gradient area */}
